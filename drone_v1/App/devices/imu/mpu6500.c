...
{
    tx_buf[0] = MPU6500_ACCEL_XOUT_H | 0x80;  // Read flag
    HAL_GPIO_WritePin(SPI_NCS_PORT, SPI_NCS_PIN, GPIO_PIN_RESET);
    HAL_SPI_TransmitReceive_DMA(SPI_BUS, tx_buf, rx_buf, 15);
}
...
void MPU6500_ProcessData(float accel[3], float gyro[3])
{
    ...
    accel_raw[0] = (int16_t)((rx_buf[1] << 8) | rx_buf[2]);
    accel_raw[1] = (int16_t)((rx_buf[3] << 8) | rx_buf[4]);
    accel_raw[2] = (int16_t)((rx_buf[5] << 8) | rx_buf[6]);

    gyro_raw[0] = (int16_t)((rx_buf[9] << 8) | rx_buf[10]);
    gyro_raw[1] = (int16_t)((rx_buf[11] << 8) | rx_buf[12]);
    gyro_raw[2] = (int16_t)((rx_buf[13] << 8) | rx_buf[14]);

    accel[0] = (float)(accel_raw[0] - accel_offset[0]) / cfg.accel_sens;
    accel[1] = (float)(accel_raw[1] - accel_offset[1]) / cfg.accel_sens;
    accel[2] = (float)(accel_raw[2] - accel_offset[2]) / cfg.accel_sens;

    gyro[0] = (float)(gyro_raw[0] - gyro_offset[0]) / cfg.gyro_sens;
    gyro[1] = (float)(gyro_raw[1] - gyro_offset[1]) / cfg.gyro_sens;
    gyro[2] = (float)(gyro_raw[2] - gyro_offset[2]) / cfg.gyro_sens;
}
